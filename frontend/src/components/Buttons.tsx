...
export const PrimaryButton: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({ children, className, ...props }) => (
    <button className={`inline-flex items-center justify-center gap-2 rounded-full px-5 py-2 text-sm font-medium bg-[#96BA34] text-white hover:opacity-90 active:scale-95 transition cursor-pointer ${className}`} {...props} >
        {children}
    </button>
);
